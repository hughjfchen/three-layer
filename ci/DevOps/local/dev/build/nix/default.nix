...
let
        ...
        packages = pkgs.haskell.packages // {
          "${compiler}" = pkgs.haskell.packages."${compiler}".override {
            overrides = haskellPackagesNew: haskellPackagesOld: rec {
              ...
              servant-proto-lens =
                haskellPackagesNew.callPackage ./servant-proto-lens.nix { };

              postgresql-simple-named =
                haskellPackagesNew.callPackage ./postgresql-simple-named.nix { };

              typerep-map =
                haskellPackagesNew.callPackage ./typerep-map.nix { };

              co-log =
                haskellPackagesNew.callPackage ./co-log.nix { };

              three-layer =
                haskellPackagesNew.callPackage ./three-layer.nix { };
            };
          };
        };
      };
  ...
  three-layer = pkgs.haskell.packages.${compiler}.three-layer;

  static-three-layer = pkgs.haskell.lib.justStaticExecutables pkgs.haskell.packages.${compiler}.three-layer;

  passwd = ''
    root:x:0:0::/root:/run/current-system/sw/bin/bash
    three-layer:x:90001:90001::/var/empty:/run/current-system/sw/bin/nologin
  '';

  group = ''
    root:x:0:
    nogroup:x:65534:
    three-layer:x:90001:three-layer
  '';
...
'';
  ...
  three-layer-env = stdenv.mkDerivation {
    name = "three-layer-env";
    phases = [ "installPhase" "fixupPhase" ];

    installPhase = ''
      mkdir -p $out/etc/three-layer
      echo '${three-layer-conf}' > $out/etc/three-layer/three-layer.conf
  ...
  three-layer-docker =  mybuildImage {
  name = "three-layer";
  tag = three-layer.version;
  
  contents = [ static-three-layer
               three-layer-env ];
  config = {
    Env = [
      "PATH=${static-three-layer}/bin:$PATH"
      "PARA1="
      "PARA2="
    ];
    User = "three-layer";
    Cmd = [ "three-layer" "/etc/three-layer/three-layer.conf" ];
    ExposedPorts = {
      "5432/tcp" = {};
    };
    WorkingDir = "/data";
...
in  {
  inherit three-layer;
  inherit static-three-layer;
  inherit three-layer-docker;
}
